...
if __name__ == "__main__":
    ...
    if Informations_dict["Homo-oligomer"] != 1 : #select proteins who can create an homo-oligomer
        Use_RoseTTA_PPI(HInt_object, Informations_dict, "RF2_homo_int", GPU) #set better interactions all_vs_bait #Maybe remove
    if Informations_dict["Interact_with"] != "" :
        if len(HInt_object.get_possible_prey()) > 15 :
           if Informations_dict["Organism"] == "euk" :
              Use_RoseTTA_PPI(HInt_object, Informations_dict, "RF2_PPI_int", GPU) #set better interactions all_vs_bait
           if Informations_dict["Organism"] in ["gram+","gram-"] :
              Use_RoseTTA_Lite(HInt_object, Informations_dict, GPU)      
