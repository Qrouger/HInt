...
def main() :
    ...
    print("job_with_vram_length", job_with_vram_length)
    first_need_msa = []
    first_need_pkl = []
    ...
    if (len(first_need_msa) > 0 or len(first_need_pkl) > 0) :
    ...
    if Informations_dict["Interact_with"] != [''] :
        for bait in Informations_dict["Multimer_bait"] :
            if HInt_object.get_compounds() != {} :
                ...
                target=Generate_3D_model, args=(Informations_dict, "PPI_int", job_with_vram_length, GPU, multi_job_per_gpu))
                gpu_thread.start()
